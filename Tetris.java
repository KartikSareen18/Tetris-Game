...
class Board {
	int frameX=20,frameY=21;
    public char board[][] = new char[frameX][frameY];
    public char finalBoard[][] = new char[frameX][frameY];
	
	Board() {
        for (int i = 0; i < frameX; i++) {
            for (int j = 0; j < frameY; j++) {
                if (i == 0 || i == frameX-1 || j == 0 || j == frameY-2) {
                    finalBoard[i][j] = '*';
                } else
                    finalBoard[i][j] = ' ';
            }
    ...
    void savepoint(int[][] co) {
        ...
        for (int i = 0; i < 4; i++) {
            finalBoard[co[i][0] + 4][frameY-1]++;
        }
        for (int i = 0; i < frameX; i++) {
            if (finalBoard[i][frameY-1] - '0' == frameY-3) {
                for (int m = i; finalBoard[m][frameY-1] > '0'; m--) {
                    for (int n = 1; n < frameY-2; n++) {
